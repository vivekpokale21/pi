...
def parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(description="Serve CodeRankEmbed through a small OpenAI-compatible CPU embeddings API.")
...
def main() -> None:
	...
	def embeddings(request: EmbeddingsRequest) -> dict[str, Any]:
		...
		vectors = model.encode(texts, batch_size=max(1, args.batch_size), convert_to_numpy=True, normalize_embeddings=True)
		return {
			"object": "list",
			"model": args.model,
			"data": [
