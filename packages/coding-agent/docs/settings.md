...
| `retry.maxAgentDelayMs` | number | `60000` | Maximum agent-level retry delay |
| `retry.provider.timeoutMs` | number | SDK default | Provider/SDK request timeout in milliseconds |
| `retry.provider.maxRetries` | number | `0` | Provider/SDK retry attempts |
| `retry.provider.maxRetryDelayMs` | number | `60000` | Max server-requested delay before failing (60s) |

Agent-level exponential backoff is capped by `retry.maxAgentDelayMs`. When a provider requests a retry delay longer than `retry.provider.maxRetryDelayMs`, the request fails immediately with an informative error instead of waiting silently. Set the provider limit to `0` to disable it.
